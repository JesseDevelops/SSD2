...
namespace SSDAssignment1.Data
{
    public static class DbInitializer
    {
        ...
        {
            // Create Admin User
            var firstManager = new ApplicationUser
            {
                UserName = "TeamManager@mohawkcollege.ca",
                Email = "TeamManager@mohawkcollege.ca",
                FirstName = "The",
                LastName = "Manager",
                EmailConfirmed = true,
            };
            var result = await userManager.CreateAsync(firstManager, appSecrets.AdminPwd);
            if (!result.Succeeded)
                return 1;  // should log an error message here

            // Assign user to Admin role
            result = await userManager.AddToRoleAsync(firstManager, "Manager");
            if (!result.Succeeded)
                return 2;  // should log an error message here
            ...
            {
                UserName = "FirstPlayer@mohawkcollege.ca",
                Email = "FirstPlayer@mohawkcollege.ca",
                FirstName = "The",
                LastName = "Player",
                EmailConfirmed = true
            };
